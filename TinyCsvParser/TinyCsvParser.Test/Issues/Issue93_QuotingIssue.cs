...
public class Issue93_QuotingIssue
{
    ...
    {
        var csvParserOptions = new CsvParserOptions(skipHeader: false, tokenizer: new StringSplitTokenizer(['|'], false));
        var csvReaderOptions = new CsvReaderOptions([Environment.NewLine]);
        var csvMapper = new SomeDtoMapping();
        var csvParser = new CsvParser<SomeDto>(csvParserOptions, csvMapper);
